...
export default function NotebookViewPage() {
  ...
  const handleSave = useCallback(
    async (config: NotebookConfig) => {
      if (!id) return

      setSaveStatus('saving')
      try {
        await updateNotebook.mutateAsync({ id: Number.parseInt(id, 10), config })
        setSaveStatus('saved')
  ...
  const handleDashboardSaved = useCallback(
    async (data: { title: string; description?: string; dashboardConfig: any }) => {
      try {
    ...
    [createDashboard, navigate, connectionId]
  )
            ...
            <div className="relative">
              ...
              {showSettings && (
                <div className="absolute right-0 top-full mt-2 z-50 w-[calc(100vw-2rem)] max-w-96 bg-dc-surface rounded-lg shadow-xl border border-dc-border p-4">
                  ...
                  <label className="block text-xs font-medium text-dc-text-secondary mb-1">
                    Provider Endpoint{' '}
                    <span className="text-dc-text-muted font-normal">(optional)</span>
                  </label>
                  <input
                    type="text"
                    value={endpoint}
                    onChange={e => {
                      ...
                      setEndpoint(v)
                      v
                        ? localStorage.setItem(ENDPOINT_STORAGE_KEY, v)
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-medium text-dc-text-secondary hover:text-dc-text"
                    >
                      Done
                    </button>
                  </div>
