...
export default function DashboardListPage() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (await confirm({ title: 'Delete dashboard', message: `Are you sure you want to delete "${name}"?`, confirmText: 'Delete', variant: 'danger' })) {
      try {
          ...
          {pages.map((page) => (
            ...
            >
              ...
              <div className="p-6 relative">
                ...
                <h3 className="text-lg font-medium text-dc-text leading-tight pr-28">{page.name}</h3>

                {page.description && (
                  <p className="text-sm text-dc-text-muted line-clamp-2 mt-1 mb-2">{page.description}</p>
                )}

                <div className="flex items-center flex-wrap text-xs text-dc-text-muted mt-2 gap-x-3 gap-y-1 mb-3">
                  <span>{page.config.portlets.length} portlet{page.config.portlets.length !== 1 ? 's' : ''}</span>
          ...
          <div className="bg-dc-surface rounded-xl shadow-xl max-w-md w-full p-6 border border-dc-border">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  onClick={() => { setIsNewModalOpen(false); setNewName(''); setNewDescription(''); setNewConnectionId(undefined) }}
